...
public:
	DynamicPlotter(std::string name_) : name(name_), W(cvplot::Window::current(name_)) {
		W.offset({ 60, 100 });
	}

	void setPlotTitle(unsigned int n, std::string plotName) {
		auto title = name + "_" + std::to_string(n);

		cvplot::Size siz(xSize, ySize);
		cvplot::View view = W.view(title, siz);
		view.offset(cvplot::Offset(0, n * ySize));
		view.title(plotName);
		cvplot::Figure figure = cvplot::Figure(view);


		//auto &figure = cvplot::Figure(view);


		//auto &figure = cvplot::figure(title);
		//figure.square(true);
		figure.origin(false, false);
		srand(clock());
		auto x = 0.f, y = 0.f, dx = 1.f, dy = 0.f, f = 0.f, df = 0.f;
		figure.series(std::to_string(n) + "random").dynamicColor(true).legend(false);
		clock_t time = 0;
		for (int i = 0; i < 100; i++) {
			auto fps = CLOCKS_PER_SEC / (float)(clock() - time);
			time = clock();
			...
			f = (f + df) * 0.8f;
			df = (df + rand() % 11 / 100.f - .05f) * 0.8f;
			figure.series(std::to_string(n) + "random").add(x += dx, { y += dy, i / 10.f });
			figure.show(false);
			auto string = std::to_string(fps).substr(0, 4) + " fps  " +
				std::to_string(i / 10.f).substr(0, 4) + "%";
			view.drawText(string, { 480, 277 }, cvplot::Gray);
	...
	void sg() {
		...
		cvplot::Window::current("a").offset({ 60, 100 });
